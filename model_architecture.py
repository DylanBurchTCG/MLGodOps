...
class MultiTaskCascadedLeadFunnelModel(nn.Module):
    ...
    """
    Fixed forward pass that maintains gradient flow during training
    while using cascade selection during inference.
    """
    ...
    if is_training:
        # TRAINING PATH: Process all inputs through all stages to maintain gradient flow
        # We still compute indices for loss weighting, but don't use them for filtering
        
        # Process all samples through applied stage
        applied_features = self.transformer_applied(shared_features)
        applied_pred = self.applied_head(applied_features)

        # Determine applied selection size
        if self.use_percentages:
            k_applied = max(1, int(k_toured * self.applied_pct))
        else:
            k_applied = min(self.applied_k, k_toured)
            
        # Get applied indices for monitoring/weighting only
        if applied_pred.squeeze().dim() == 0:
            applied_indices = torch.tensor([0], device=device)
        else:
            # Add noise to break ties
            noise = torch.randn_like(applied_pred.squeeze()) * 1e-6
            _, applied_indices = torch.topk(applied_pred.squeeze() + noise, k_applied)

        # Process all samples through rented stage
        rented_features = self.transformer_rented(shared_features)
        rented_pred = self.rented_head(rented_features)

        # Determine rented selection size
        if self.use_percentages:
            k_rented = max(1, int(k_applied * self.rented_pct))
        else:
            k_rented = min(self.rented_k, k_applied)
            
        # Get rented indices for monitoring/weighting only
        if rented_pred.squeeze().dim() == 0:
            rented_indices = torch.tensor([0], device=device)
        else:
            # Add noise to break ties
            noise = torch.randn_like(rented_pred.squeeze()) * 1e-6
            _, rented_indices = torch.topk(rented_pred.squeeze() + noise, k_rented)
            
    else:
        # INFERENCE PATH: Use cascaded filtering between stages
        # This doesn't need to be differentiable
        ...
        if len(toured_indices) == 0:
            ...
            if applied_pred_subset.squeeze().dim() == 0:
                ...
                # NEW: Determine k for applied stage
                if self.use_percentages:
        ...
        if len(applied_indices) == 0:
            ...
            if rented_pred_subset.squeeze().dim() == 0:
                ...
                # NEW: Determine k for rented stage
                if self.use_percentages:
